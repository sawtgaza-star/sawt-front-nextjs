"use client";
/* Loads /pages/media/contact once, in the browser — same reasoning as
   ./use-media-page (static export, so a build-time fetch would freeze the copy
   ...
   so an outage settles into a page that is just its banner shell rather than
   bars that shimmer forever. The error is logged, not surfaced. */

import { useEffect, useState } from "react";
import { fetchMediaContactPage, type MediaContactPage } from "./media-contact";

export type MediaContactState = {
  page: MediaContactPage | null;
  loading: boolean;
};

export function useMediaContact(): MediaContactState {
  ...
  const [state, setState] = useState<MediaContactState>({ page: null, loading: true });

  useEffect(() => {
    const controller = new AbortController();

    fetchMediaContactPage(controller.signal)
      .then((page) => setState({ page, loading: false }))
      .catch((caught) => {
        // The unmount aborted it — leave the state alone, nothing is watching.
        if (caught?.name === "AbortError") return;
        console.warn("[media contact] no content to show:", caught);
