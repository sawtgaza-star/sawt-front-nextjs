"use client";
/* Loads /pages/media once, in the browser — same reasoning as ./use-about-page
...
import { fetchMediaPage, type MediaPage } from "./media-page";
...
export function useMediaPage(): MediaPageState {
  ...
  const [state, setState] = useState<MediaPageState>({ page: null, loading: true });

  useEffect(() => {
    const controller = new AbortController();

    fetchMediaPage(controller.signal)
      .then((page) => setState({ page, loading: false }))
      .catch((caught) => {
        // The unmount aborted it — leave the state alone, nothing is watching.
        if (caught?.name === "AbortError") return;
        console.warn("[media] no content to show:", caught);
