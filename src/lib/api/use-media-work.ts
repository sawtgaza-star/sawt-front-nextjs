"use client";
/* Loads one case study in the browser — same reasoning as ./use-media-page
...
import { fetchMediaWork, type MediaWorkPage } from "./media-work";

export type MediaWorkState = {
  page: MediaWorkPage | null;
  loading: boolean;
};

export function useMediaWork(work: string): MediaWorkState {
  ...
  const [state, setState] = useState<MediaWorkState>({ page: null, loading: true });

  useEffect(() => {
    const controller = new AbortController();
    setState({ page: null, loading: true });

    fetchMediaWork(work, controller.signal)
      .then((page) => setState({ page, loading: false }))
      .catch((caught) => {
        // The unmount aborted it — leave the state alone, nothing is watching.
        if (caught?.name === "AbortError") return;
        console.warn(`[media work ${work}] no case study to show:`, caught);
